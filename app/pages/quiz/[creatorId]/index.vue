<template>
  <main class="w-screen h-dvh bg-primary-950 flex flex-col">
    ...
    <header class="h-16 border-b border-primary-800">
      ...
      >
        <div>
          <button
            type="button"
            class="button--primary button--icon size-10!"
          >
            <Icon
    ...
    >
      <div class="flex items-center gap-4">
        <Icon
          ...
          size="32"
          class="text-ascend-purple-dark"
        />
        <span class="text-lg font-semibold text-ascend-purple-dark">Please wait for round to load...</span>
      </div>
    </div>
    <div
      v-else-if="!quiz"
    ...
    >
      <div
        class="container mx-auto px-15 grid grid-cols-[minmax(0,1fr)_320px] gap-15"
      >
        <div class="space-y-6">
          <div class="space-y-1.5">
            <div
              class="text--secondary font-medium flex items-center justify-between"
            >
            ...
            <div>
              <div
                class="w-full h-2 bg-primary-900 rounded-full overflow-hidden"
              >
                <div
          ...
          <UiCard class="space-y-6">
            <div
              class="grid grid-cols-[150px_minmax(0,1fr)_150px] items-center"
            >
              <div>
                <span
                  class="bg-ascend-purple-dark py-1.5 px-3 text-xs rounded-full"
                >{{ currentQuestion?.points }} points</span>
              </div>
              <div class="text-center">
                <div
                  class="flex items-center gap-2 justify-center text-red-500"
                >
                  <Icon
              ...
              <div class="flex justify-end">
                <span
                  class="bg-primary-800 py-1.5 px-3 text-xs rounded-full capitalize"
          ...
          <UiCard class="space-y-3">
            ...
            <div class="space-y-4">
              ...
              >
                ...
                <div class="text-ascend-purple-dark font-semibold">
                  0
                </div>
              </UiCard>
              <UiCard
...
const routeParams = computed(
  () => route.params as { creatorId: string, quizId: string },
)
...
watch(
  ...
  (connected) => {
    if (connected) {
      console.debug('WebSocket connected, joining quiz...')
      if (!joinedQuiz.value) {
        onJoinQuiz({
          nickname: nickname.value,
          creatorId: Number.parseInt(creatorId.value),
          quizId: Number.parseInt(quizId.value),
        })
        joinedQuiz.value = true
      } else {
        console.warn(
          'Already joined quiz, not sending joinQuiz message again.',
        )
      }
    } else {
...
const { data: quiz, status } = useFetch<TQuiz>(
  `/api/quizzes/${creatorId.value}/${quizId.value}`,
  {
...
const currentQuestion = computed(
  () => quiz.value?.questions[currentRound.value - 1] || null,
) // Placeholder for current question, should come from quiz data
const completedPercentage = computed(
  () => (currentRound.value / maxRounds.value) * 100,
)
</script>
